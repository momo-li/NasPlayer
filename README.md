...
基于 HarmonyOS（ArkTS/ArkUI）的 NAS 媒体播放器：自动发现局域网中的 SMB 主机，也支持通过 IPv6 或域名连接远程主机，浏览共享目录并直接播放视频/音频、查看图片，无需挂载或下载。
...
- 共享与目录浏览：列出共享、文件夹、视频、图片与音频文件，支持多级目录返回
...
- 图片查看：全屏查看图片，上下滑切换同目录图片，双指缩放（1x~4x）与双击放大/还原，沉浸式全屏
- 音频播放：上一曲/下一曲（按钮 + 上下滑 + 播完自动续播）、进度拖拽、倍速、循环模式（顺序/单曲/列表/随机）
- 音频后台播放：接入 AVSession 播控中心（下拉快捷菜单 / 锁屏）显示标题、封面与播放控制，后台长时任务保持播放
- 音乐信息：解析 ID3 标题 / 艺术家 / 专辑与内嵌封面，无标签时回退文件名
- 文件管理：长按条目弹出操作面板，支持删除（二次确认）与重命名（需服务端支持 SMB2 `SET_INFO`）
...
AVPlayer 通过 `http://127.0.0.1:<port>/stream?f=<path>` 拉流，`StreamServer` 将 Range 请求转换为 SMB 的 `readRange` 读取，实现边下边播；图片则经 SMB 读取整文件后由 `ImageLoader` 解码为 PixelMap，音频元数据经 `AudioMetaUtil` 解析。
...
│   ├── BrowserPage.ets    # 目录与文件浏览（删除/重命名入口）
│   ├── PlayerPage.ets     # 视频播放页
│   ├── ImageViewerPage.ets# 图片查看页（滑动切换/缩放）
│   └── AudioPlayerPage.ets# 音频播放页（切歌/元数据/播控中心）
├── smb/                   # SMB2 协议实现
│   ├── SmbClient.ets      # 连接、readdir/getSize/readRange/删除/重命名
...
│   ├── MediaUtil.ets      # 视频/图片/音频格式识别、MIME、格式化
│   ├── ImageLoader.ets    # 图片读取 + PixelMap 解码 + LRU 缓存
│   ├── AudioMetaUtil.ets  # ID3 元数据与封面解析 + 缓存
...
3. 选择共享目录，进入文件夹浏览视频、音频与图片文件
4. 点击视频开始播放：上下滑切换视频，左右滑快进/快退，长按 X2 倍速，单击显示/隐藏控制栏
5. 点击图片进入查看页：上下滑切换同目录图片，双指缩放、双击放大/还原，单击显示/隐藏顶栏
6. 点击音频进入播放页：上一曲/下一曲（按钮或上下滑）、拖动进度、倍速与循环模式；切到后台或锁屏后
   可在播控中心（下拉快捷菜单）查看与控制播放
7. 长按任意条目弹出操作面板：
   - 删除：点击「删除」后需二次确认；文件夹仅支持删除空文件夹
   - 重命名：输入新名称即可（部分 NAS 的 SMB 服务未实现 `SET_INFO`，此时会提示不支持）
8. 下拉列表可刷新当前目录
...
| 类型 | 格式 |
|------|------|
| 视频 | mp4、mkv、avi、mov、ts、m2ts、webm、wmv、flv、m4v、3gp、rmvb、rm、mpg、mpeg、vob |
| 图片 | jpg、jpeg、jfif、png、gif、webp、bmp、heif、heic、avif、ico（GIF 仅显示首帧） |
| 音频 | mp3、m4a、aac、wav、ogg、oga、flac、amr |
...
| `ohos.permission.KEEP_BACKGROUND_RUNNING` | 音频后台播放的长时任务（配合 `backgroundModes: ["audioPlayback"]`） |
